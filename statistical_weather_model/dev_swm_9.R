...
l_m = load_pca_analysis(variable = "mean_sea_level_pressure",
                        descriptive_string = "nordic")

dt_fl_daily = l_m$dt_factor_loadings[, .("fl_daily" = mean(V1)), date]
...
for(y in seq_along(years)){
    X11()
    par(mfrow = c(1,2))
    dt_focus = dt_fl_m[date %in% as.Date(paste0(years[y], "-11-15")):as.Date(paste0(years[y] + 1, "-03-15"))]
    plot(dt_focus[, .(date, fl_daily)], type = "l", main = years[y], ylim = dt_fl_m[, range(fl_daily)], ylab = "U Wind")
